...
(function(g){ 'use strict';
  ...
  function start(el){
    ...
    for(var i=0;i<count;i++){
      var x, y, z, len;
      do { x = __hf.random()*2-1; y = __hf.random()*2-1; z = __hf.random()*2-1; len = x*x+y*y+z*z; }
      while(len > 1 || len === 0);
      var r = Math.cbrt(__hf.random());
      positions[i*3] = x*r; positions[i*3+1] = y*r; positions[i*3+2] = z*r;
      randoms[i*4] = __hf.random(); randoms[i*4+1] = __hf.random(); randoms[i*4+2] = __hf.random(); randoms[i*4+3] = __hf.random();
      var col = hexToRgb(palette[Math.floor(__hf.random()*palette.length)]);
    ...
    var red = reduced(), lastTime = __hf.now(), elapsed = red ? 2000 : 0, rotZ = 0;
    function frame(t){
      ...
      if(!red) __hf.requestAnimationFrame(frame);
    }
