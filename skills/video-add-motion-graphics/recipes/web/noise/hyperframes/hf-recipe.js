...
(function(g){ 'use strict';
  ...
  function start(el){
    ...
    function drawGrain(){
      ...
      for(var i=0;i<data.length;i+=4){ var v = __hf.random()*255; data[i]=v; data[i+1]=v; data[i+2]=v; data[i+3]=DEF.patternAlpha; }
      ctx.putImageData(imageData, 0, 0);
    }
    if(reduced()){ drawGrain(); return; }
    (function loop(){ if(frame % DEF.patternRefreshInterval === 0) drawGrain(); frame++; __hf.requestAnimationFrame(loop); })();
  }
