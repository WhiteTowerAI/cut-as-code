...
(function(g){ 'use strict';
  ...
  function run(el){ var target=el.getAttribute('data-text')||el.textContent; if(red()){ el.textContent=target; return; }
    var frame=0, id; __hf.clearInterval(el.__d); el.__d=__hf.setInterval(function(){ var out='';
      for(var i=0;i<target.length;i++){ if(i < frame/2){ out+=target[i]; } else if(target[i]===' '){ out+=' '; } else { out+=CH[Math.floor(__hf.random()*CH.length)]; } }
      el.textContent=out; frame++; if(frame/2>=target.length){ __hf.clearInterval(el.__d); el.textContent=target; } }, 30); }
