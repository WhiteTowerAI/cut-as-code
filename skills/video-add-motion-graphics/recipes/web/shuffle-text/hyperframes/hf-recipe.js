...
(function(g){ 'use strict';
  ...
  function run(el){ var target=el.getAttribute('data-text')||el.textContent; if(red()){ el.textContent=target; return; }
    var f=0; __hf.clearInterval(el.__s); el.__s=__hf.setInterval(function(){ var out='';
      for(var i=0;i<target.length;i++){ if(f > i*3+6){ out+=target[i]; } else if(target[i]===' '){ out+=' '; } else { out+=AZ[Math.floor(__hf.random()*26)]; } }
      el.textContent=out; f++; if(f > target.length*3+6){ __hf.clearInterval(el.__s); el.textContent=target; } }, 28); }
