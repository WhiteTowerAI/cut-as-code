...
(function (global) {
  ...
  function scrambleTo(el, newText) {
    ...
    for (var i = 0; i < len; i++) {
      var from = oldText[i] || '';
      var to = newText[i] || '';
      var start = Math.floor(__hf.random() * 20);
      var end = start + Math.floor(__hf.random() * 20) + 8;
      queue.push({ from: from, to: to, start: start, end: end, char: '' });
    }
    if (el.__scrambleRaf) __hf.cancelAnimationFrame(el.__scrambleRaf);
      ...
      function update() {
        ...
        for (var i = 0; i < queue.length; i++) {
          ...
          if (frame >= q.end) { done++; out += q.to; }
            ...
            if (!q.char || __hf.random() < 0.28) q.char = CHARS[Math.floor(__hf.random() * CHARS.length)];
        ...
        frame++;
        el.__scrambleRaf = __hf.requestAnimationFrame(update);
      }
      update();
    });
  }
...
function (__hf) {
    ...
    var phrases = ['motion, anything', 'launch, faster', 'ship, tasteful'];
    var i = 0, el = document.querySelector('.scramble');
    document.getElementById('again').addEventListener('click', function () {
      i = (i + 1) % phrases.length; window.scrambleTo(el, phrases[i]);
    });
