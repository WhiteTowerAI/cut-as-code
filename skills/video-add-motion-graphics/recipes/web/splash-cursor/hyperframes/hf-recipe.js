...
(function(g){ 'use strict';
  ...
  function start(el){
    ...
    updateKeywords();
    initFramebuffers();
    let lastUpdateTime = __hf.dateNow();
    ...
    function updateFrame() {
      ...
      step(dt);
      render(null);
      animationFrameId.current = __hf.requestAnimationFrame(updateFrame);
    }

    function calcDeltaTime() {
      let now = __hf.dateNow();
    ...
    function clickSplat(pointer) {
      ...
      color.g *= 10.0;
      color.b *= 10.0;
      let dx = 10 * (__hf.random() - 0.5);
      let dy = 30 * (__hf.random() - 0.5);
    ...
    function generateColor() {
      ...
      let c = HSVtoRGB(__hf.random(), 1.0, 1.0);
      c.r *= 0.15;
      c.g *= 0.15;
      c.b *= 0.15;
      return c;
...
function (__hf) {
/* demo driver (preview only): swirl a synthetic pointer so the fluid shows without a real mouse */
(function(){ if(matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  var t0=__hf.now();
  function swirl(t){ var e=(t-t0)/1000; if(e>2.6) return;
    var cx=innerWidth/2, cy=innerHeight/2, r=Math.min(cx,cy)*0.45;
    var x=cx+Math.cos(e*3.1)*r, y=cy+Math.sin(e*4.3)*r*0.7;
    window.dispatchEvent(new MouseEvent(e<0.05?'mousedown':'mousemove',{clientX:x,clientY:y,bubbles:true}));
    __hf.requestAnimationFrame(swirl); }
  __hf.requestAnimationFrame(swirl);
})();
}
];
