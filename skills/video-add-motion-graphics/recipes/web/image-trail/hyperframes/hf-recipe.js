...
(function(g){ 'use strict';
  ...
  function attach(el){ if(el.__it) return; el.__it=1; var imgs=(el.getAttribute('data-images')||'').split(',').filter(Boolean); var k=0, last=0;
    ...
    if(off()) return;
    el.addEventListener('pointermove', function(e){ var now=__hf.dateNow(); if(now-last<80) return; last=now; var r=el.getBoundingClientRect();
      ...
      el.appendChild(n); k++; n.animate([{opacity:.9,transform:'translate(-50%,-50%) scale(1)'},{opacity:0,transform:'translate(-50%,-50%) scale(.8)'}],{duration:700,easing:'ease-out'}); __hf.setTimeout(function(){ n.remove(); },700); }); }
