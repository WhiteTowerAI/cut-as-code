...
(function(g){ 'use strict';
  ...
  function attach(el){ if(el.__rot) return; el.__rot=1; var words=(el.getAttribute('data-words')||el.textContent).split('|'); var i=0;
    ...
    __hf.setInterval(function(){ i=(i+1)%words.length; if(red()){ w.textContent=words[i].trim(); return; } el.classList.add('swap');
      __hf.setTimeout(function(){ w.textContent=words[i].trim(); el.classList.remove('swap'); }, 460); }, 2200); }
