...
function (__hf) {
/* Deterministic HyperFrames canvas port of the tsParticles fireworks preset; see upstream-options.ts. */
const canvas = document.getElementById("sticker-canvas");
const ctx = canvas.getContext("2d");
const width = canvas.width;
const height = canvas.height;
const profile = "fireworks";
...
const particles = Array.from({ length: profile === "links" ? 64 : profile === "fireflies" ? 80 : 150 }, (_, index) => ({
  index,
  x: __hf.random() * width,
  y: __hf.random() * height,
  vx: (__hf.random() - 0.5) * 95,
  vy: (__hf.random() - 0.5) * 95,
  speed: 110 + __hf.random() * 240,
  angle: __hf.random() * Math.PI * 2,
  phase: __hf.random() * 4.8,
  life: 1.2 + __hf.random() * 2.2,
  radius: 2 + __hf.random() * 5,
