...
def _selection_errors(cue_id, cue, recipe_ids, require_known_fields=True):
    ...
    for candidate in shortlist if isinstance(shortlist, list) else []:
        ...
        if (
            ...
            or any(
                not _nonblank(field)
                or (require_known_fields and field not in allowed_matched_fields)
...
def _recipe_errors(
    ...
    expected_prefix = f"work/cache/motion-graphics/hyperframes/{cue_id}/"
...
def _adaptation_errors(cue_id, cue, project_root=None, verify_files=False, required=False):
    ...
    expected_prefix = f"work/cache/motion-graphics/adapted/{cue_id}/"
    for binding in files:
...
def _frame_errors(cue_id, cue, render, program, timeline, project_root, verify_files, media_size):
    ...
    if (
        ...
        and pattern_valid
        and render.get("asset") == f"cache/motion-graphics/rendered/{cue_id}"
    ):
...
def validate_plan(
    ...
    for cue in cues:
        ...
        if (
            ...
            or render.get("asset") != f"cache/motion-graphics/rendered/{cue_id}"
...
def register_operation(project, plan, timeline, project_root, *, plan_path="motion-graphics/motion-graphics-plan.json"):
    ...
    old = [item for item in result["operations"] if isinstance(item, dict) and item.get("id") == "motion-graphics"]
    ...
    if not renders:
        if old:
            result["operations"] = [item for item in result["operations"] if item.get("id") != "motion-graphics"]
            sequence["operations"] = [item for item in sequence["operations"] if item != "motion-graphics"]
            result.setdefault("render", {})["status"] = "draft"
        return result
    has_timeline_insert = any(item.get("kind") == "timeline-insert" for item in renders)
    common = {
        "id": "motion-graphics",
        "skill": "video-add-motion-graphics",
        "status": "verified",
        ...
        "target": {"sequence": result["active_sequence"], "scope": "motion-graphics"},
        "effects": {
            "changes_timeline": has_timeline_insert,
            "changes_geometry": False,
            "changes_video_pixels": True,
            "changes_audio": has_timeline_insert,
            "adds_track": "motion-graphics",
        },
        "render": renders,
        "check": {"status": "pass", "report": "../review/04-motion-graphics/motion-graphics-summary.md"},
    }
    expected = {**common, "revision": old[0].get("revision") if len(old) == 1 else None}
    if len(old) == 1 and old[0] == expected and sequence["operations"].count("motion-graphics") == 1:
        return result
    revision = max((item.get("revision", 0) for item in old), default=0) + 1
    result["operations"] = [item for item in result["operations"] if item.get("id") != "motion-graphics"]
    result["operations"].append({**common, "revision": revision})
    ids = [item for item in sequence["operations"] if item != "motion-graphics"]
    anchors = [
    ...
    ids.insert(index, "motion-graphics")
