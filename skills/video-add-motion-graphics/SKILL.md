...
region across every overlapping cue as a reserved exclusion zone. Motion graphics runs after
...
[example-motion-graphics-plan.json](examples/example-motion-graphics-plan.json).
...
4. Build a deterministic local shortlist:
   ...
   node skills/video-add-motion-graphics/scripts/recipe_library.mjs show decrypted-text --json
   ```
...
6. Materialize the chosen preconverted recipe:
   ...
   ```powershell
   node skills/video-add-motion-graphics/scripts/recipe_library.mjs materialize decrypted-text `
...
7. Set `authoring_mode: recipe-adaptation`. Author the real composition under
   `work/cache/motion-graphics/adapted/<cue-id>/`, using the recipe's recognizable motion mechanic
   ...
   **Information budget:** Every visible element must earn its place by carrying the cue's
   ...
   ```powershell
   node skills/video-add-motion-graphics/scripts/recipe_library.mjs bind-adaptation `
     --project D:\path\to\video-project --cue gm-001 --json
   ```
...
10. Bind timeline, transcript, understanding, media, contact sheet, recipe files, adaptation
   ...
   receipt. Run
   `validate_plan(..., verify_files=True)`, write `motion-graphics-plan.json`, then call
