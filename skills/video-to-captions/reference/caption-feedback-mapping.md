...
agent maps that request to `caption-style.ts` or preview candidates. Do not ask
the user to edit `Caption.tsx`.

## Safe Edit Points

- Use `examples/caption-style.ts` for the final confirmed style.
- Use `examples/preview-config.ts` for preview candidates.
- Use `examples/captions.json` only when correcting subtitle text/timing data.
- Keep `examples/Caption.tsx` and other renderer files stable for ordinary style
  feedback.
