...
function (__hf) {
delete globalThis.parcelRequire;
delete globalThis.parcelRequire_;
    ...
    function polyfill() {
      ...
      function step(context) {
        ...
        if (currentX !== context.x || currentY !== context.y) {
          w.__hf.requestAnimationFrame(step.bind(w, context));
        }
      }
      /**
...
var _default$1 = /*#__PURE__*/function (_Core) {
  ...
  _createClass(_default, [{
    ...
    value: function addSpeed() {
      if (window.pageYOffset != this.instance.scroll.y) {
        this.instance.speed = (window.pageYOffset - this.instance.scroll.y) / Math.max(1, __hf.dateNow() - this.speedTs);
      } else {
        this.instance.speed = 0;
      }
    }
  ...
  (function () {
    ...
    root.Lethargy = function () {
      ...
      Lethargy.prototype.isInertia = function (direction) {
        ...
        if (this.deltasTimestamp[this.stability * 2 - 2] + this.delay > __hf.dateNow() && lastDeltas[0] === lastDeltas[this.stability * 2 - 1]) {
          return false;
        }
  ...
  _createClass(_default, [{
    ...
    value: function addSpeed() {
      if (this.instance.delta[this.directionAxis] != this.instance.scroll[this.directionAxis]) {
        this.instance.speed = (this.instance.delta[this.directionAxis] - this.instance.scroll[this.directionAxis]) / Math.max(1, __hf.dateNow() - this.speedTs);
      } else {
        this.instance.speed = 0;
      }
    }
...
},{}],"ReQy":[function(require,module,exports) {
"use strict";
...
var images = _toConsumableArray(document.querySelectorAll('.content__img')); // Preload images and fonts


Promise.all([(0, _utils.preloadImages)(), (0, _utils.preloadFonts)('api4rrx')]).then(function () {
  // Remove loader (loading class)
  document.body.classList.remove('loading'); // keep track of the previous and current scroll values
  ...
  lscroll.on('scroll', function (obj) {
    ...
    scroll.cache = scroll.current; // stretch the words when scrolling down

    var scaleY = (0, _utils.map)(distance, 0, 150, 1, 3);
    ...
    try {
      for (_iterator.s(); !(_step = _iterator.n()).done;) {
        ...
        word.el.style.transform = "scale3d(1,".concat(scaleY, ",1)");
    ...
    var scale = (0, _utils.map)(Math.abs(distance), 0, 150, 1, 0.8);
    images.forEach(function (imgEl) {
      return imgEl.style.transform = "scale3d(".concat(scale, ",").concat(scale, ",1)");
    });
  }); // set up the parallax movement of the chars individually (need to set the Locomotive scroll data-scroll-speed option)

  var _iterator2 = _createForOfIteratorHelper(splitting.entries()),
      _step2;
  ...
  try {
    for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
      ...
      if (word.el.classList.contains('content__text')) {
        var charsTotal = word.chars.length;

        var _iterator3 = _createForOfIteratorHelper(word.chars.entries()),
            _step3;
...
},{"./utils":"MgTz","locomotive-scroll":"ez7q","splitting/dist/splitting.css":"oR66","splitting/dist/splitting-cells.css":"yIBK","splitting":"Qdhu"}]},{},["ReQy"], null)
}
];
