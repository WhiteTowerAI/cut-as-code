---
name: line-md-switch-twotone-to-switch-off-twotone-transition
description: Adapt the bundled Line MD Switch Twotone To Switch Off Twotone Transition sticker from Line MD as a selective HyperFrames overlay.
---

# Line MD Switch Twotone To Switch Off Twotone Transition

Use the preconverted `hyperframes/` composition as the immutable base. Preserve the recognizable
source motion, keep every animated pixel outside visible faces, and remove any element that does not
carry the cue's meaning.
