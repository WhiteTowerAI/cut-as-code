  ...
  window.HPX['neural-net'] = function(el){
    ...
    const stop = U.loop((t) => {
      ...
      if (t - last > 0.25){
        ...
        const s = starts[(__hf.random()*starts.length)|0];
      ...
      for (const p of pulses){
        ...
        if (p.t >= 1){
          ...
          p.node2 = next[(__hf.random()*next.length)|0];
      ...
      for (const p of pulses){
        if (!p.target){
          const next = nodes.filter(n=>n.L===p.L+1);
          p.target = next[(__hf.random()*next.length)|0];
        }
        p.t += 0.04;
...
function (__hf) {
    ...
    if (!window.matchMedia || !matchMedia('(prefers-reduced-motion: reduce)').matches) {
      HPX['neural-net'](document.querySelector('.stage'));
    }
