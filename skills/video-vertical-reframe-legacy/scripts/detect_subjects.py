#!/usr/bin/env python3
"""Phase 1 person detection for video-vertical-reframe.
...
This is the migrated subject-detection layer only. It does not plan crops,
render vertical video, avoid subtitles, or make reframe decisions.
"""
...
def load_yolo(model_name, model_cache_dir):
    ...
    try:
        ...
        # Keep weights under the run output, not the repo root.
...
def validate_tracks_contract(data):
    ...
    if producer.get("skill") != "video-vertical-reframe":
        fail("tracks.json producer.skill must be video-vertical-reframe")
    video = data.get("video", {})
    for key in ["source", "width", "height", "fps", "duration_s"]:
        if key not in video:
            fail(f"tracks.json video.{key} is required")
    for frame in data.get("frames", []):
        ...
        for subject in frame["subjects"]:
            for key in ["subject_id", "track_id", "class", "bbox", "bbox_kind", "confidence"]:
                if key not in subject:
                    fail(f"tracks.json subject missing {key}")
            if subject["class"] != PERSON_CLASS_NAME:
                fail("Phase 1 tracks may only contain class=person")
    for track in data.get("tracks", []):
        for key in ["track_id", "class", "start_s", "end_s", "num_observations", "mean_confidence", "coverage"]:
            if key not in track:
                fail(f"tracks.json track missing {key}")
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="vertical-subject-detect-") as tmp:
        ...
        tracks_json = {
            ...
            "metadata": {
                ...
                "phase": "Phase1 Subject Detection only; no reframe planning is performed",
            },
        }
        validate_tracks_contract(tracks_json)
    ...
    metrics = {
        ...
        "outputs": {
            ...
            "contact_sheet": str(artifacts / "contact_sheet.jpg"),
        },
    }
