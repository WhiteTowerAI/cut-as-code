#!/usr/bin/env python3
"""Render Phase 1 subject-detection debug artifacts.
...
This is a detection review artifact only. It does not plan crops, write crop
windows, or create a final vertical video.
"""
...
def load_tracks(path):
    ...
    producer_skill = data.get("producer", {}).get("skill")
    if producer_skill not in {"video-subject-detect", "video-vertical-reframe"}:
        fail("tracks.json producer.skill must be video-subject-detect or video-vertical-reframe")
    for frame in data.get("frames", []):
        if "time_s" not in frame or "frame_index" not in frame or "subjects" not in frame:
            fail("tracks.json frames must include time_s, frame_index, subjects")
    return data
...
def draw_frame(frame_path, frame_spec, max_width):
...
def summarize_review_issues(tracks):
    ...
    lines = []
    lines.append("- NO SUBJECT frames: " + (", ".join(f"{t:.2f}s" for t in empty[:12]) if empty else "none"))
    lines.append("- Multi-subject frames: " + (", ".join(f"{t:.2f}s ({n})" for t, n in multi[:12]) if multi else "none"))
    lines.append("- Low-confidence detections (<0.50): " + (
        ", ".join(f"{t:.2f}s {track_id}={conf:.2f}" for t, track_id, conf in low_conf[:12])
        if low_conf else "none"
    ))
    lines.append("- Large center jumps: " + (
        ", ".join(f"{track_id} {a:.2f}s->{b:.2f}s ({dist:.0f}px)" for track_id, a, b, dist in jumps[:12])
        if jumps else "none"
    ))
    lines.append("- If preview boxes look wrong at these times, compare against contact_sheet.jpg first.")
    return lines


def write_summary(out_path, tracks_path, source, tracks, preview_path):
    frames = tracks["frames"]
    ...
    lines = [
        "# video-vertical-reframe Phase1 subject detection summary",
        "",
        "## Inputs",
        "",
        f"- source: `{source}`",
        ...
        "## Review Notes",
        "",
        "- This is a subject-detection debug artifact only.",
        "- This phase does not create `reframe_plan.json`, `crop_path.json`, or vertical video.",
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="vertical-subject-preview-") as tmp:
        tmpdir = Path(tmp)
        ...
        for i, frame in enumerate(tracks["frames"], start=1):
            ...
            drawn = draw_frame(raw, frame, args.max_width)
    ...
    write_summary(summary, tracks_path, source, tracks, preview)
