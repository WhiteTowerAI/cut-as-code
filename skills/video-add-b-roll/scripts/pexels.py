...
def search_videos(query, *, orientation="landscape", per_page=10, api_key=None, opener=None):
    ...
    for video in payload["videos"]:
        ...
        for item in video.get("video_files", []):
            ...
            if not isinstance(item, dict) or file_key in seen_files: continue
        ...
        try:
            with _open(opener, request) as response:
                ...
                if offset and status == 206:
                    ...
                    if not match or int(match.group(1)) != offset or int(match.group(2)) < offset or int(match.group(3)) <= int(match.group(2)): raise ValueError("invalid Content-Range")
                    mode = "ab"
                elif offset and status == 200: mode = "wb"; offset = 0
                elif status != 200: raise ValueError("unexpected download status")
                else: mode = "wb"
                length = headers.get("Content-Length")
                if length is not None and (not str(length).isdigit() or offset + int(length) > max_bytes): raise ValueError("download exceeds limit")
                total = offset
...
def import_local(source, destination, provenance):
    ...
    if not all(isinstance(provenance.get(key), str) and provenance[key].strip() for key in ("creator", "license", "retrieval_time", "original_path")): raise ValueError("provenance is incomplete")
